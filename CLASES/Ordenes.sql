...
SELECT * FROM tabla_prueba;
ALTER TABLE tabla_prueba RENAME COLUMN genero TO genero2;
ALTER TABLE tabla_prueba MODIFY genero2 VARCHAR(50);
UPDATE  tabla_prueba  SET genero2="M2" WHERE  genero2="M";
SELECT * FROM tabla_prueba ORDER BY nombre;
DELETE FROM tabla_prueba WHERE nombre="Adrián";
SELECT Ocupacion FROM tabla_prueba GROUP BY Ocupacion;
SELECT * FROM employee_sons;
SELECT tp.nombre,emp.nombre FROM tabla_prueba AS tp INNER JOIN employee_sons AS emp ON tp.CC =emp.CC;
SELECT Ocupacion ,AVG(salario) FROM tabla_prueba GROUP BY Ocupacion;
SELECT nombre, AVG(edad) FROM tabla_pr;